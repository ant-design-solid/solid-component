...
describe("measureTextAreaHeight", () => {
  it("measures natural height for content-box textareas", () => {
    ...
    const result = measureTextAreaHeight(textarea);
  ...
  it("uses placeholder text when the textarea value is empty", () => {
    ...
    expect(result.overflowY).toBeUndefined();
  });
  ...
  it("clamps border-box height with minRows and maxRows", () => {
    ...
    const result = measureTextAreaHeight(textarea, 2, 3);
  ...
  it("enforces minRows even when content is shorter than the minimum", () => {
    ...
    const result = measureTextAreaHeight(textarea, 3);

    expect(result.height).toBe(42);
    expect(result.minHeight).toBe(42);
