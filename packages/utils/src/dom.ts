 ...
 */
export default function findDOMNode<T = Element | Text>(_node: MaybeAccessor<HTMLElement | SVGElement>): T | null {
...
export function getDOM(elementRef: MaybeAccessor) {
  ...
  const dom = findDOMNode(el) || (el && typeof el === 'object' ? findDOMNode((el as any).nativeElement) : null)
...
export function isVisible(element: Element) {
  ...
  if (element instanceof Element) {
    if ((element as HTMLElement).offsetParent) return true
